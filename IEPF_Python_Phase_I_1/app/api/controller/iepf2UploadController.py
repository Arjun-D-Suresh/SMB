...
class IEPF2Controller:
    ...
    def insert_excel_data(self, file_paths, file_type):
        ...
        try:
            for fp in file_paths:
                ...
                if file_extension == 'xlsx' or file_extension == 'xls':
                    ...
                    try:
                        ...
                        if(len(columns)>16 and (self.db_columns[17] in excel_data.columns)):
                        ...
                        if(result[0] == 0 and result[1] == 0): status = "No dividend found for this file"
                        elif(result[1] > 0): status = "File contains multiple dividend rows"
                        elif(result[0] > 0): 
                            status = "File contains only single dividend rows"
                             # process single dividend data
                            processer_result = self.iepf2model.iepf2_processer()

                        print("\t-", file_name, "ended   :", datetime.now())
                        print("\n\t--------------------------------------------------------------\n")
                        results.append({'file' : file_name, 'status' : status, 'data' : [result]})

                    except (ValueError, IndexError) as e:
                        results.append({'file' : file_name, 'status' : str(e)})
            ...
            return {'message':str(e), 'data':results}, 500

    def insert_multiple_dividend(self, cin,log_id,xfer_date,division,dividend_id):
        return {
            'result': 'success'
        } if(self.iepf2model.multiple_dividend_processor(cin,log_id,xfer_date,division,dividend_id)) else ({
            'result': 'something went wrong'
        }, 500)
    
    def get_multiple_dividend(self, cin, security_code, log_id, xfer_date, division, skip, take):
        
        result = self.iepf2model.get_multiple_dividend(cin, security_code, log_id, xfer_date, division, skip, take)
        return result
